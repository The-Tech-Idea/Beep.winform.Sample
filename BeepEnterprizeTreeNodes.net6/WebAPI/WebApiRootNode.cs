...
{
    [AddinAttribute(Caption = "Web API", Name = "WebApiRootNode.Beep", misc = "Beep", iconimage = "webapi.ico", menu = "Beep",ObjectType ="Beep")]
       ...
       // public event EventHandler<PassedArgs> ActionNeeded;
        ...
        public IErrorsInfo CreateNodes()
        {
            ...
            try
            {
                ...
                foreach (ConnectionProperties i in DMEEditor.ConfigEditor.DataConnections.Where(c => c.Category == DatasourceCategory.WEBAPI && c.CompositeLayer == false))
                {
                    if (TreeEditor.treeBranchHandler.CheckifBranchExistinCategory(i.ConnectionName, "WEBAPI") == null)
                    {
                        CreateWebApiNode(i.ConnectionName);
                        i.Drawn = true;
                    }
